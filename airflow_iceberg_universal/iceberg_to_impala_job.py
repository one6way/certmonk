#!/usr/bin/env python3
"""
Spark Job для чтения Iceberg таблицы из Hive и записи в Impala

Airflow Variables:
- ICEBERG_BUCKET, ICEBERG_SCHEMA, ICEBERG_TABLE
- hive_metastore_uri, s3_endpoint
- aws_access_key_id, aws_secret_access_key
- IMPALA_CONN_ID (connection ID в Airflow)
- IMPALA_TARGET_SCHEMA, TARGET_IMPALA_TABLE
...
def get_impala_connection():
    """Получить параметры подключения к Impala"""
    impala_conn_id = os.getenv('IMPALA_CONN_ID', 'impala_default')
    
    host = os.getenv('IMPALA_HOST')
    port = os.getenv('IMPALA_PORT', '21050')
    database = os.getenv('IMPALA_DATABASE')
    user = os.getenv('IMPALA_USER')
    password = os.getenv('IMPALA_PASSWORD')
    
    if not all([host, database, user, password]):
        raise ValueError("Missing Impala connection parameters")
    
    url = f"jdbc:hive2://{host}:{port}/{database}"
...
def write_to_impala(df, url, properties, schema, table):
    """Записывает DataFrame в Impala"""
    full_table = f"{schema}.{table}"
    write_mode = os.getenv('WRITE_MODE', 'append')
    
    print(f"\nWriting to Impala: {full_table}")
...
def main():
    ...
    target_schema = os.getenv('IMPALA_TARGET_SCHEMA', 'analytics')
    target_table = os.getenv('TARGET_IMPALA_TABLE', 'iceberg_data')
    
    print(f"{'='*60}")
    print(f"Iceberg to Impala Pipeline")
    print(f"{'='*60}")
    ...
    spark = (SparkSession.builder
        .appName(f"iceberg_to_impala_{schema}_{table}")
        .master(spark_master)
    ...
    try:
        ...
        url, properties = get_impala_connection()
        
        write_to_impala(df, url, properties, target_schema, target_table)
