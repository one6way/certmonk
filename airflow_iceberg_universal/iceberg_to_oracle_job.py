#!/usr/bin/env python3
"""
Spark Job для чтения Iceberg таблицы из Hive и записи в Oracle

Airflow Variables:
- ICEBERG_BUCKET, ICEBERG_SCHEMA, ICEBERG_TABLE
- hive_metastore_uri, s3_endpoint
- aws_access_key_id, aws_secret_access_key
- ORACLE_CONN_ID (connection ID в Airflow)
- TARGET_ORACLE_SCHEMA, TARGET_ORACLE_TABLE
...
def get_oracle_connection():
    """Получить параметры подключения к Oracle"""
    oracle_conn_id = os.getenv('ORACLE_CONN_ID', 'oracle_default')
    ...
    if not all([host, sid, user, password]):
        raise ValueError("Missing Oracle connection parameters")
...
def write_to_oracle(df, url, properties, schema, table):
    """Записывает DataFrame в Oracle"""
    full_table = f"{schema}.{table}".upper()
    write_mode = os.getenv('WRITE_MODE', 'append')
    
    print(f"\nWriting to Oracle: {full_table}")
...
def main():
    ...
    target_schema = os.getenv('TARGET_ORACLE_SCHEMA', 'DATA_SCHEMA')
    target_table = os.getenv('TARGET_ORACLE_TABLE', 'ICEBERG_DATA')
    
    print(f"{'='*60}")
    print(f"Iceberg to Oracle Pipeline")
    print(f"{'='*60}")
    ...
    spark = (SparkSession.builder
        .appName(f"iceberg_to_oracle_{schema}_{table}")
        .master(spark_master)
    ...
    try:
        ...
        url, properties = get_oracle_connection()
        
        write_to_oracle(df, url, properties, target_schema, target_table)
